...
import com.ansorkazama.udacity_baking_recipe_apps.presenter.RecipeDetailPresenterViewContract;
import com.ansorkazama.udacity_baking_recipe_apps.databinding.FragmentRecipeDetailBinding;
import com.ansorkazama.udacity_baking_recipe_apps.model.Ingredient;
...
import com.ansorkazama.udacity_baking_recipe_apps.adapter.StepAdapter;
...
public class RecipeDetailFragment extends Fragment implements RecipeDetailPresenterViewContract.View {
    ...
    private final String INS_KEY_ADAPTER_POSITION = "instance_key_adapter_position";
    private final String INS_KEY_RECIPE = "instance_key_recipe";
    private final String INS_KEY_INGREDIENTS_ID = "instance_key_ingredients_id";
    private final String INS_KEY_INGREDIENTS_COUNT = "instance_key_ingredients_count";
    private final String INS_KEY_STEPS = "instance_key_steps";
    ...
    public static RecipeDetailFragment newInstance(Recipe recipe) {
        ...
        bundle.putParcelable(BDL_RECIPE_DATA, recipe);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if ((arguments != null) && (arguments.containsKey(BDL_RECIPE_DATA))) {
            mRecipe = arguments.getParcelable(BDL_RECIPE_DATA);
        }
        if(savedInstanceState != null && savedInstanceState.containsKey(INS_KEY_ADAPTER_POSITION)) {
            mStepAdapterSavedPosition = savedInstanceState.getInt(INS_KEY_ADAPTER_POSITION);
        }
        if(savedInstanceState != null) {
            if(savedInstanceState.containsKey(INS_KEY_RECIPE)) {
                mRecipe = savedInstanceState.getParcelable(INS_KEY_RECIPE);
            }
            if(savedInstanceState.containsKey(INS_KEY_STEPS)) {
                mStepList = savedInstanceState.getParcelableArrayList(INS_KEY_STEPS);
            }
            if(savedInstanceState.containsKey(INS_KEY_INGREDIENTS_COUNT)) {
                int ingredientCount = savedInstanceState.getInt(INS_KEY_INGREDIENTS_COUNT);
                for(int i=0; i<ingredientCount; i++) {
                    ...
                    checkBox.setChecked(savedInstanceState.getBoolean(INS_KEY_INGREDIENTS_ID + String.valueOf(i)));
...
//        ((AppCompatActivity) getActivity()).setSupportActionBar(binding.tbToolbar.toolbar);
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt(INS_KEY_ADAPTER_POSITION, mStepAdapter.getStepAdapterCurrentPosition());
        outState.putParcelable(INS_KEY_RECIPE, mRecipe);
        outState.putParcelableArrayList(INS_KEY_STEPS, mStepList);
        outState.putInt(INS_KEY_INGREDIENTS_COUNT, mIngredientList.size());
        for(int ingedientId = 0; ingedientId<mIngredientList.size(); ingedientId++) {
                    outState.putBoolean(INS_KEY_INGREDIENTS_ID + String.valueOf(ingedientId),
